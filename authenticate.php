...
$stmt = $con->prepare('SELECT id, email,firstname, password, rememberme, activation_code, role, profit, registered_date, amount_invested, earnings_mon, customer_id  FROM accounts WHERE username = ?');
...
if ($stmt->num_rows > 0) {
	$stmt->bind_result($id, $email, $firstname, $password, $rememberme, $activation_code, $role, $profit, $registered_date, $amount_invested, $earnings_mon, $customer_id);
	$stmt->fetch();
	$stmt->close();
	...
	if (password_verify($_POST['password'], $password)) {
		...
		if (account_activation && $activation_code != 'activated') {
			...
			$_SESSION['registered_date'] = $registered_date;
			$_SESSION['amount_invested'] = $amount_invested;
			$_SESSION['earnings_mon'] = $earnings_mon;
